...
import DelegationTeamCards from "@/features/countries/DelegationTeamCards";
...
export default function CountryDelegationsView({
  ...
  return (
    <div className="space-y-12">
      {delegations.map((d) => {
        const fieldSize = d.edition.contestants.filter((c) => c.status !== "unofficial").length;
        ...
        return (
          <section key={d.edition.slug} className="space-y-6">
            ...
            <div>
              ...
              <DelegationTeamCards contestants={d.contestants} fieldSize={fieldSize} />
            </div>
          </section>
        );
      })}
